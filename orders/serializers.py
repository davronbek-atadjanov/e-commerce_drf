...
from orders.models import CartItem, Discount, DeliveryTariff, Order
...
class OrderDetailSerializer(serializers.ModelSerializer):
    items = CartItemListSerializer(many=True)
    delivery_tariff = DeliveryTariffSerializer(many=True)
    discount = DiscountSerializer(many=True)

    class Meta:
        model = Order
        fields = ("id", "status","total_price","address","discount", "items", "payment_status", "payment_method", "delivery_tariff")


class OrderDiscountSerializer(serializers.Serializer):
    order_id = serializers.IntegerField(required=True)
    discount = DiscountSerializer(required=True)
